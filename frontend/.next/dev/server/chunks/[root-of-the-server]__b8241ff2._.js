...
"[project]/lib/prebakedQueries.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
"[project]/lib/destinationImageCatalog.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prebakedQueries$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/prebakedQueries.ts [app-route] (ecmascript)");
;
...
const learnedDestinations = new Map();
function normalizeToAsciiLower(text) {
...
function translateToEnglishTokens(destination) {
    const normalized = normalizeToAsciiLower(destination);
    if (!normalized) return "";
    let translated = normalized;
    for (const [token, replacement] of Object.entries(tokenDictionary)){
        translated = translated.replace(new RegExp(`\\b${escapeRegex(token)}\\b`, "g"), replacement);
    }
    return translated.replace(/\s+/g, " ").trim();
}
function getLocationDetails(destination) {
    const normalized = normalizeToAsciiLower(destination || "");
...
function buildLocationPriorityQueries(location, presetHints) {
    const queries = [];
    const { city, country, englishCity, englishCountry, combined, englishCombined } = location;
    const addQuery = (value)=>{
        if (value) queries.push(value.trim());
    };
    addQuery(englishCombined);
    if (englishCountry && englishCity && `${englishCountry} ${englishCity}`.trim() !== englishCombined) {
    ...
    landscapeHints.forEach((hint)=>{
        [
            ...
            englishCountry,
            country
        ].filter(Boolean).forEach((value)=>addQuery(`${value} ${hint}`));
    });
    return queries.filter(Boolean);
}
function buildDestinationQueries(destination, preset = "neutral") {
    const translated = translateToEnglishTokens(destination || "");
    const locationDetails = getLocationDetails(destination);
    const parts = translated.split(",").map((p)=>p.trim()).filter(Boolean);
    const primary = parts[0] || translated || destination.trim() || "";
    const secondary = parts.length > 1 ? parts[1] : "";
    const extraFromPrebaked = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prebakedQueries$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getPrebakedQueries"])(destination);
    ...
    const locationQueries = buildLocationPriorityQueries(locationDetails, presetHints);
    const contextual = [];
...
function learnDestination(destination, queries, imageUrl) {
    if (!destination || !imageUrl) return;
    const key = normalizeKey(destination);
    if (!key || learnedDestinations.has(key)) return;
    learnedDestinations.set(key, {
        queries,
        imageUrl
    });
}
}),
"[project]/app/api/destination-image/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/lib/destinationImageCatalog.ts [app-route] (ecmascript)");
;
;
...
async function GET(req) {
    ...
    const queries = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["buildDestinationQueries"])(qRaw, preset);
    ...
    if (pexelsResult?.length) {
        ...
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["learnDestination"])(qRaw, usedQuery ? [
            usedQuery
        ] : queries, picked);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            url: picked,
...
function buildScoringTokens(destination, query, preset) {
    const translated = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["translateToEnglishTokens"])(destination || "");
    const destWords = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeToAsciiLower"])(translated).split(/\s|,/).filter(Boolean);
    const queryWords = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeToAsciiLower"])(query).split(/\s|,/).filter(Boolean);
    const presetHints = preset === "mountains" ? [
        "mountains",
        "landscape"
    ] : preset === "beach" || preset === "tropical" ? [
...
function buildFallbackPexelsQueries(destination, preset, exclude = []) {
    const location = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getLocationDetails"])(destination);
...
function pickBestByTokens(photos, tokens = []) {
    ...
    const scored = photos.map((photo)=>{
        const alt = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$destinationImageCatalog$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["normalizeToAsciiLower"])(photo.alt || "");
        let score = 0;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b8241ff2._.js.map